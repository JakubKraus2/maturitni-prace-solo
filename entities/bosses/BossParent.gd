...
func _process(delta):
	if SwordsMaster.get_node("AnimationPlayerSpawn").current_animation == "SPAWN":
		...
		_ready()
		get_parent().get_node("BossArenaBarrier").visible = false
		get_parent().get_node("BossArenaBarrier").get_node("CollisionShape2D").set_deferred("disabled", true)
		get_parent().get_node("BossArenaBarrier2").visible = false
		get_parent().get_node("BossArenaBarrier2").get_node("CollisionShape2D").set_deferred("disabled", true)
