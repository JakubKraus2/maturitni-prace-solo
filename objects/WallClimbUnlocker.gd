...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("ui_accept") && get_overlapping_areas() && game_data.can_wall_climb == false:
		game_data.can_wall_climb = true
		SaveFile.save_data()
...
func _on_Unlocker_area_entered(area):
	if game_data.can_wall_climb == false:
