...
func _process(delta):
	if Input.is_action_just_pressed("ui_accept") && get_overlapping_areas():
		SpecialSkillMenu.get_node("Skills1/Panel1").disabled = false
		SpecialSkillMenu.get_node("Skills2/Panel1").disabled = false
		SpecialSkillMenu.get_node("Skills3/Panel1").disabled = false
		
		SpecialSkillMenu.skills.skills_panel1_disabled = false
